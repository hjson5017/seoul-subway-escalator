...
def load_data():
    ...
    # CSV가 있으면 CSV 우선 사용, 없으면 xlsx 자동 탐색
    csv_candidates = glob.glob(os.path.join(base_dir, "*.csv"))
    xlsx_candidates = glob.glob(os.path.join(base_dir, "*.xlsx"))
    ...
    if not xlsx_candidates:
        st.error(f"데이터 파일(csv 또는 xlsx)을 찾을 수 없습니다. 현재 폴더 파일 목록: {os.listdir(base_dir)}")
        st.stop()
